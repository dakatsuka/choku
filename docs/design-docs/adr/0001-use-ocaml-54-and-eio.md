...
Camelio is starting as a new HTTP server project for OCaml 5. The project slogan
...
Camelio targets OCaml 5.4 and Eio. It will not depend on `cohttp`, `lwt`, or
`async`.

## Alternatives Considered

- Build on `cohttp`: rejected because it would make Camelio a wrapper around an
  existing HTTP stack.
