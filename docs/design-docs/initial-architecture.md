...
- `Choku.Client`: future HTTP client API with separate client request and
  response values.
...
- `Choku.Request`: HTTP server request values.
- `Choku.Response`: HTTP server response values.
...
`Choku.Method`, `Choku.Headers`, `Choku.Status`, and `Choku.Body` form the
shared protocol value layer where their contracts fit both client and server.
`Choku.Request` and `Choku.Response` are server/application values. Future
client request and response values belong under `Choku.Client`. Server-specific
lifecycle concerns belong in `Choku.Server`; future client connection pooling,
redirect policy, and TLS configuration should belong in `Choku.Client`.

HTTP version-specific parsing, framing, stream multiplexing, flow control, and
transport integration should stay outside the protocol value layer. HTTP/1.1
...
appropriate request/response values where that mapping is valid.
...
server request/response values and do not depend on protocol-specific parser
state. Future HTTP/2 and HTTP/3 modules should translate to or from those
server values where their protocol semantics allow it. Client modules should use
their own request/response values while sharing lower-level protocol primitives.
