...
let request ?content_type body =
  let body = Choku.Body.string body in
  let headers =
    match content_type with
    | None -> Choku.Headers.empty
    | Some value -> Choku.Headers.add "content-type" value Choku.Headers.empty
  in
  Choku.Request.make ~meth:Choku.Method.POST ~target:"/upload" ~headers ~body
...
let multipart_error = of_pp Choku.Multipart.pp_error
...
let with_temp_dir name fn =
  ...
  let dir =
    Eio.Path.(
      Eio.Stdenv.fs env
      / ("/tmp/choku-multipart-" ^ string_of_int (Unix.getpid ()) ^ "-" ^ name))
  in
...
let expect_multipart = function
  ...
  | Error error ->
      fail
        (Format.asprintf "expected multipart, got %a" Choku.Multipart.pp_error
           error)

let part_body part = Choku.Body.to_string (Choku.Multipart.Part.body part)

let test_filename_sanitize () =
  let sanitize = Choku.Multipart.Filename.sanitize in
...
let test_tempfile_save_source () =
  ...
  let saved =
    Choku.Multipart.Tempfile.save_source ~dir ~random:(random_source 0)
      ~original_filename:"foo/bar.jpg"
      (Eio.Flow.string_source "hello")
  in
  let expected_name = "choku-upload-00000000000000000000000000000000.tmp" in
  check (option string) "basename" (Some expected_name)
    (Eio.Path.split (Choku.Multipart.Tempfile.path saved) |> Option.map snd);
  check (option string) "original" (Some "foo/bar.jpg")
    (Choku.Multipart.Tempfile.original_filename saved);
  check (option string) "display" (Some "foo-bar.jpg")
    (Choku.Multipart.Tempfile.display_filename saved);
  check int "size" 5 (Choku.Multipart.Tempfile.size saved);
  check string "body" "hello"
    (Eio.Path.load (Choku.Multipart.Tempfile.path saved));
...
let test_tempfile_save_source_retries_on_collision () =
  ...
  let first_name = "choku-upload-00000000000000000000000000000000.tmp" in
  let second_name = "choku-upload-02020202020202020202020202020202.tmp" in
  Eio.Path.save ~create:(`Exclusive 0o600)
    Eio.Path.(dir / first_name)
    "existing";
  let saved =
    Choku.Multipart.Tempfile.save_source ~dir
      ~random:
        (random_sources
...
let test_tempfile_save_source_cleans_up_on_failure () =
  ...
  check_raises "copy failure" (Failure "copy failed") (fun () ->
      ignore
        (Choku.Multipart.Tempfile.save_source ~dir ~random:(random_source 1)
           (failing_source ())
          : _ Choku.Multipart.Tempfile.t));
  check (list string) "entries" [] (Eio.Path.read_dir dir)

let file_part () =
  let multipart =
    Choku.Multipart.decode ~boundary multipart_body |> expect_multipart
  in
  match Choku.Multipart.get "file" multipart with
  | Some part -> part
...
let test_tempfile_save_part () =
  ...
  let saved =
    Choku.Multipart.Tempfile.save_part ~dir ~random:(random_source 255)
      (file_part ())
  in
  ...
  check (option string) "display" (Some "hello.txt")
    (Choku.Multipart.Tempfile.display_filename saved);
  check int "size" 10 (Choku.Multipart.Tempfile.size saved);
...
let test_part_save_to_path () =
  ...
  let path =
    Eio.Path.(
      Eio.Stdenv.fs env
      / ("/tmp/choku-multipart-part-" ^ string_of_int (Unix.getpid ()) ^ ".txt"))
  in
  Fun.protect
    ~finally:(fun () -> try Eio.Path.unlink path with _ -> ())
    (fun () ->
      Choku.Multipart.Part.save_to_path ~create:(`Or_truncate 0o600) path
        (file_part ());
...
let test_get_and_get_all_preserve_order () =
  let body =
    "--AaB03x\r\n\
     ...
     --AaB03x--\r\n"
  in
  let multipart = Choku.Multipart.decode ~boundary body |> expect_multipart in
  check (option string) "first tag" (Some "ocaml")
    (Choku.Multipart.get "tag" multipart |> Option.map part_body);
  check (list string) "all tags" [ "ocaml"; "eio" ]
    (Choku.Multipart.get_all "tag" multipart |> List.map part_body)
...
let test_quoted_parameter_semicolon () =
  ...
  | Some part ->
      check (option string) "filename" (Some "a;b.txt")
        (Choku.Multipart.Part.filename part)
...
let test_filename_sanitize_path_traversal_from_part () =
  ...
  | Some part ->
      ...
      check string "sanitized" "ssh-id_rsa"
        (Choku.Multipart.Part.filename part
        |> Option.value ~default:"" |> Choku.Multipart.Filename.sanitize)
...
let test_of_request_extracts_quoted_boundary () =
  let request =
    request
      ...
      multipart_body
  in
  let multipart = Choku.Multipart.of_request request |> expect_multipart in
  check int "part count" 2 (List.length (Choku.Multipart.parts multipart))
...
let test_of_request_limited_rejects_malformed_body () =
  ...
  check
    (result reject multipart_error)
    "malformed" (Error Choku.Multipart.Malformed_body)
    (Choku.Multipart.of_request_limited ~max_size:20 request)

let test_of_request_limited_rejects_unexpected_end_of_body () =
  let body =
    Choku.Body.Internal.streaming
  ...
  check
    (result reject multipart_error)
    "unexpected end" (Error Choku.Multipart.Unexpected_end_of_body)
    (Choku.Multipart.of_request_limited
       ~max_size:(String.length multipart_body)
       request)
...
let test_of_request_limited_rejects_negative_max_size () =
  ...
  check_raises "negative max_size" (Invalid_argument "negative max_size")
    (fun () ->
      ignore
        (Choku.Multipart.of_request_limited ~max_size:(-1) request
          : (Choku.Multipart.t, Choku.Multipart.error) result))

let test_of_request_limited_rejects_negative_max_size_before_headers () =
  check_raises "negative max_size" (Invalid_argument "negative max_size")
    (fun () ->
      ignore
        (Choku.Multipart.of_request_limited ~max_size:(-1)
...
let test_of_request_rejects_missing_content_type () =
  check
    (result reject multipart_error)
    "missing content-type" (Error Choku.Multipart.Missing_content_type)
    (Choku.Multipart.of_request (request multipart_body))

let test_of_request_rejects_unsupported_content_type () =
  check
    (result reject multipart_error)
    "unsupported content-type"
    (Error (Choku.Multipart.Unsupported_content_type "application/json"))
    (Choku.Multipart.of_request
       (request ~content_type:"application/json" multipart_body))

let test_of_request_rejects_missing_boundary () =
  check
    (result reject multipart_error)
    "missing boundary" (Error Choku.Multipart.Missing_boundary)
    (Choku.Multipart.of_request
       (request ~content_type:"multipart/form-data" multipart_body));
  check
    (result reject multipart_error)
    "empty boundary" (Error Choku.Multipart.Missing_boundary)
    (Choku.Multipart.of_request
...
let test_of_request_limited_rejects_content_type_errors () =
  check
    (result reject multipart_error)
    "missing content-type" (Error Choku.Multipart.Missing_content_type)
    (Choku.Multipart.of_request_limited ~max_size:100 (request multipart_body));
  check
    (result reject multipart_error)
    "unsupported content-type"
    (Error (Choku.Multipart.Unsupported_content_type "application/json"))
    (Choku.Multipart.of_request_limited ~max_size:100
       (request ~content_type:"application/json" multipart_body));
  check
    (result reject multipart_error)
    "missing boundary" (Error Choku.Multipart.Missing_boundary)
...
let streaming_request ?content_type body =
  let body =
    Choku.Body.Internal.streaming ~content_length:(String.length body)
...
let chunked_streaming_request ?content_type ~content_length chunks =
  ...
  in
  let body = Choku.Body.Internal.streaming ~content_length source in
  request_with_body ?content_type body
    ...
    Choku.Multipart.Streaming.iter_request request ~on_part:(fun part source ->
        ...
        seen :=
          Printf.sprintf "%s|%s|%s|%s"
            (Option.value ~default:"" (Choku.Multipart.Streaming.name part))
            (Option.value ~default:"" (Choku.Multipart.Streaming.filename part))
            (Option.value ~default:""
               (Choku.Multipart.Streaming.content_type part))
            body
          :: !seen)
  in
...
let test_streaming_iter_request_rejects_malformed_body () =
  ...
  check
    (result reject multipart_error)
    "malformed" (Error Choku.Multipart.Malformed_body)
    (Choku.Multipart.Streaming.iter_request request ~on_part:(fun _ _ ->
...
let test_streaming_iter_request_rejects_content_type_errors () =
  check
    (result reject multipart_error)
    "missing content-type" (Error Choku.Multipart.Missing_content_type)
    (Choku.Multipart.Streaming.iter_request (streaming_request multipart_body)
       ~on_part:(fun _ _ -> fail "handler should not run"));
  check
    (result reject multipart_error)
    "unsupported content-type"
    (Error (Choku.Multipart.Unsupported_content_type "application/json"))
    (Choku.Multipart.Streaming.iter_request
...
let test_streaming_iter_request_rejects_negative_max_header_size () =
  ...
  check_raises "negative max_header_size"
    (Invalid_argument "negative max_header_size") (fun () ->
      ignore
        (Choku.Multipart.Streaming.iter_request ~max_header_size:(-1) request
           ~on_part:(fun _ _ -> ())
          : (unit, Choku.Multipart.error) result))
...
let test_streaming_iter_request_rejects_large_headers () =
  ...
  check
    (result reject multipart_error)
    "large headers" (Error Choku.Multipart.Malformed_body)
    (Choku.Multipart.Streaming.iter_request ~max_header_size:10 request
       ~on_part:(fun _ _ -> fail "handler should not run"))
  ...
  List.iter
    (fun body ->
      check
        ...
        (Choku.Multipart.decode ~boundary body))
    [
      "";
